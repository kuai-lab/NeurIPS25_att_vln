...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.self_prediction_head = ClsPrediction(self.config.hidden_size, input_size=self.config.hidden_size*2)
        self.init_weights()
    ...
    def forward_navigation_per_step(
        ...
        curr_state = torch.cat([gmap_embeds[:, 0], vp_embeds[:, 0]], 1)
        # print(fuse_weights)
