...
class Seq2SeqAgent(BaseAgent):
    ...
    def end_of_episode(self, instruction, traj, obs, m_entropy, o_entropy, steps):
        ...
        if active_query:
            ...
            scan, gt_traj = self.env.gt_trajs[instruction]
            shortest_distances = self.env.shortest_distances[scan]
            path = sum(traj[0]['path'], [])
            success = bool(float(shortest_distances[path[-1]][gt_traj[-1]] < 3.0))
        else:
