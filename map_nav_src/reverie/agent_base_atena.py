...
class Seq2SeqAgent(BaseAgent):
    ...
    @torch.enable_grad()
    def end_of_episode(self, instruction, traj, obs, m_entropy, o_entropy, steps):
        episode_summary = torch.mean(torch.cat(self.curr_states, dim=0), dim=0)
        episode_result = self.vln_bert.vln_bert.self_prediction_head(episode_summary).squeeze(0)
        active_query = bool((o_entropy / steps) > self.args.active_criteria)
        loss = m_entropy / steps

        if active_query:
            self.optimizer = optim.AdamW(self.vln_bert.parameters(), self.args.lr_pre)

            scan, gt_traj, gt_objid = self.env.gt_trajs[instruction]
            goal_viewpoints = set(self.env.obj2vps['%s_%s'%(scan, str(gt_objid))])
            pred_path = sum(traj[0]['path'], [])
            success =  bool(pred_path[-1] in goal_viewpoints)
        
        else:
            self.optimizer = optim.AdamW(self.vln_bert.parameters(), self.args.lr_post)
        ...
        loss += (sf_loss*self.args.bin_weight)
