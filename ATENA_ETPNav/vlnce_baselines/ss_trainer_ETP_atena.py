...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _eval_checkpoint(
        ...
        active_criteria: float=0.05, lr_pre: float=1e-6, lr_post: float=1e-7, bin_weight: float=0.1, r_lambda: float=0.5
    ):
        if self.local_rank < 1:
        ...
        self.policy.net.rgb_encoder.eval()
        self.policy.net.depth_encoder.eval()
        self.optimizer = torch.optim.AdamW(self.policy.parameters(), lr_pre)
        ...
        while len(self.stat_eps) < eps_to_eval:
            self.rollout('eval', active_criteria, lr_pre, lr_post, bin_weight, r_lambda)
        self.envs.close()
    ...
    def rollout(self, mode, active_criteria, lr_pre, lr_post, bin_weight, r_lambda, ml_weight=None, sample_ratio=None):
        if mode == 'train':
            feedback = 'sample'
        ...
        for stepk in range(self.max_len):
            ...
            # save model's states
            curr_state = nav_outs['curr_state']
            ...
            distribution = torch.distributions.Categorical(nav_probs)
            o_entropy += distribution.entropy().item()

            # mixture entropy calculation
            mask = nav_probs[0] != 0.0
            one_hot = F.one_hot(a_t, num_classes = len(nav_probs[0]))[0][mask]
            q_dist = one_hot * r_lambda + \
                         nav_probs[0][mask] * (1-r_lambda)
            q_logq = q_dist * -torch.log(q_dist)
            m_entropy += sum(q_logq)
            t += 1
            ...
            if mode == 'eval':
                ...
                for i in range(self.envs.num_envs):
                    ...
                    episode_summary = torch.mean(torch.cat(self.curr_states, dim=0), dim=0)
                    episode_result = self.policy.net.vln_bert.binary_predictor(episode_summary).squeeze(0)
                    active_query = bool((o_entropy / t) > active_criteria)
                    loss = m_entropy / t
                    ...
                    if active_query:
                        ...
                        self.optimizer = torch.optim.AdamW(self.policy.parameters(), lr_post)
                        success = bool(torch.sigmoid(episode_result) > 0.5)

                    loss = loss if success else -loss
                    sf_loss = self.binary_criterion(episode_result, torch.tensor(float(success)).cuda())
                    loss += (sf_loss*bin_weight)
